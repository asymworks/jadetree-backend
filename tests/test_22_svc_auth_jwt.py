"""Test JSON Web Token (JWT) Handling."""
...
def test_decode_jwt(app):
    """Ensure the auth_service.decodeJwt can decode a generated token."""
    token = auth_service.encodeJwt(
        app,
        subject='urn:jadetree.auth.register'
    )
...
def test_invalid_jwt_badalgo(app):
    """Ensure an exception is raised for a JWT with invalid algorithm."""
    token = jwt.encode(
        {
            'iat':  datetime.datetime.utcnow(),
            'iss':  'urn:jadetree.auth',
...
def test_invalid_jwt_signature(app):
    """Ensure an exception is raised for an invalid JWT signature."""
    token = auth_service.encodeJwt(
        app,
        subject='urn:jadetree.auth.register'
    )
...
def test_invalid_jwt_subject_missing_encode(app):
    """Ensure a subject claim is required by encodeJwt."""
...
def test_invalid_jwt_subject_decode(app):
    """Ensure an exception is raised for a JWT missing a subject claim."""
    token = jwt.encode(
        {
            'iat':  datetime.datetime.utcnow(),
...
def test_invalid_jwt_subject_invalid(app):
    """Ensure an exception is raised for an invalid JWT subject."""
    token = auth_service.encodeJwt(
        app,
...
def test_invalid_jwt_payload_missing(app):
    """Ensure an exception is raised for a JWT missing a payload key."""
    token = auth_service.encodeJwt(
        app,
        subject='urn:jadetree.auth.login'
    )
...
def test_invalid_jwt_payload_invalid(app):
    """Ensure an exception is raised for an invalid JWT payload key."""
    token = auth_service.encodeJwt(
        app,
        subject='urn:jadetree.auth.login',
        nonce='def',
...
def test_invalid_jwt_immature(app):
    """Ensure an exception is raised for a JWT which is not yet valid."""
...
def test_invalid_jwt_expired(app):
    """Ensure an exception is raised for an expired JWT signature."""
    now = datetime.datetime.utcnow()
    token = auth_service.encodeJwt(
        app,
        subject='urn:jadetree.auth.login',
...
def test_invalid_jwt_audience(app):
    """Ensure an exception is raised for an invalid JWT audience claim."""
    token = auth_service.encodeJwt(
        app,
        subject='urn:jadetree.auth.login',
        audience='abc'
...
def test_invalid_jwt_issuer(app):
    """Ensure an exception is raised for an invalid JWT issuer claim."""
    token = auth_service.encodeJwt(
        app,
        subject='urn:jadetree.auth.login',
        issuer='abc'
...
def test_invalid_jwt_issued_at(app):
    """Ensure an exception is raised for an invalid JWT issued-at time."""
    token = jwt.encode(
        {
            'iat':  'abc',
            'iss':  'urn:jadetree.test',
...
def test_invalid_jwt_missing_issuer(app):
    """Ensure an exception is raised for a missing JWT issuer claim."""
    token = jwt.encode(
        {
            'iat':  datetime.datetime.utcnow(),
            'aud':  [ 'urn:jadetree.test' ],
...
def test_generate_jwt_default_expiration(app, session):
    """Ensure the default expiration interval is loaded."""
...
def test_generate_jwt_config_expiration(app, session):
    """Ensure token expiration interval can be set in application config."""
    config_exp = app.config.get('TOKEN_VALID_INTERVAL', None)
...
def test_generate_jwt_fixed_expiration(app, session):
    """Ensure token expiration interval can be passed as an argument."""
    config_exp = app.config.get('TOKEN_VALID_INTERVAL', None)
    if config_exp != 7200:
...
def test_generate_jwt_fixed_issued_at(app, session):
    """Ensure token issued-at can be passed as an argument."""
    now = datetime.datetime.utcnow()
    issued = now - datetime.timedelta(minutes=10)

    u = auth_service.register_user(session, 'test@jadetree.io', 'hunter2JT', 'Test User')
    token = auth_service.generate_user_token(u, 'urn:jadetree.auth.test', issued=issued)
    payload = auth_service.decodeJwt(app, token, subject='urn:jadetree.auth.test')

    iat = datetime.datetime.utcfromtimestamp(payload['iat'])

    # Ensure default expiration was assigned (+/- 1 sec)
    assert abs((now - iat).total_seconds() - 600) <= 1
