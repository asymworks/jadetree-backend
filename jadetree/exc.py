...
class ImportError(Error):
    """Base Class for Jade Tree Import Errors."""
    pass


class JwtInvalidTokenError(Error):
    """Exception raised for invalid or missing JSON Web Token data.

    This exception class is raised during :meth:`AuthService.decodeJwt`
    processing and wraps exceptions from the underlying :mod:`PyJwt <jwt>`
    framework.
...
class MultipleResults(Error):
    """Multiple results were found when exactly one was expected."""
    pass


class NoResults(Error):
    """No results were found when exactly one was expected."""
    pass
