...
class User(TimestampMixin):
    ...
    def format_date(self, date, long=False):
        '''
        Format the date value for the user's locale. If the long parameter is
        `True` then the long date format will be used, otherwise the short
        format is used.
        '''
        fmt = 'short'
        if long:
            fmt = 'long'
            if self.fmt_date_long is not None:
                fmt = self.fmt_date_long
        elif self.fmt_date_short is not None:
            fmt = self.fmt_date_short

        # Convert Arrow instance to a datetime.date
        if isinstance(date, Arrow):
            date = date.date()

        # Format date
        return format_date(date, fmt, self.locale)
